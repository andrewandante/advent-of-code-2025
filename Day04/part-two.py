...
def clean_the_map():
    ...
    for row_number, row in map.items():
        ...
        for index, item in enumerate(row):
            ## clean out x so we can start fresh
            if item == 'x':
...
## go round once
roll_count_this_round = traverse_the_map()

## if we've got any, go again
while roll_count_this_round > 0:
    total_roll_count = total_roll_count + roll_count_this_round
    ## clean out x
    clean_the_map()
    ## go again
    roll_count_this_round = traverse_the_map()
print(f"Total count: {total_roll_count}")
